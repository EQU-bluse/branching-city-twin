...
class BranchStore:
    ...
    def compare(self, name_a: str, name_b: str) -> dict[str, object]:
        ...
        branch is looked up; unknown branches raise :class:`KeyError`
        and identical names raise :class:`ValueError`. Each closure
        follows the graph's parents-before-children, ``(at, id)`` order
        with every event exactly once. The result is a fresh dict whose
        keys are ordered ``common, left_only, right_only, fork``: the
        shared event ids in the left branch's replay order, the ids
        exclusive to each side (each in its own branch's replay order),
        and the last shared id — the fork point — or ``None`` when the
        closures share nothing. The query is read-only: the graph,
        branch heads and records are never modified, and the returned
        dict and tuples are detached from internal state.
        """
        self._require_nonempty_str(name_a, "name_a")
        self._require_nonempty_str(name_b, "name_b")
        self._require_known_branch(name_a)
        ...
        if name_a == name_b:
            raise ValueError(
                f"cannot compare branch {name_a!r} with itself"
            )
        ...
        common = tuple(event_id for event_id in left_order if event_id in right_set)
        left_only = tuple(
            event_id for event_id in left_order if event_id not in right_set
        )
        right_only = tuple(
            event_id for event_id in right_order if event_id not in left_set
        )
        return {
            "common": common,
            "left_only": left_only,
